...
from app.core.pii_anonymizer import PIIAnonymizer, default_anonymizer
...
class PIIAnonymizingCallback(BaseCallbackHandler):
    ...
    注意：此回调会修改发送到 LLM 的实际输入内容，
    对邮箱、电话等敏感信息进行脱敏后再发送给模型。
    如需只脱敏日志而不影响 LLM 输入，请在业务层显式处理。
    """
                ...
                for generation in generations:
                    ...
                    if hasattr(generation, "message") and hasattr(generation.message, "content"):
                        if isinstance(generation.message.content, str):
                            generation.message.content = self.anonymizer.anonymize(generation.message.content)
