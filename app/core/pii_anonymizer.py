...
BUILTIN_PII_PATTERNS: dict[str, PIIPattern] = {
    ...
    # 中国手机号
    "phone_cn": PIIPattern(
        name="phone", pattern=re.compile(r"\b1[3-9]\d{9}\b"), mask_func=lambda x: x[:3] + "****" + x[-4:]
    ),
    # IP 地址
    "ip": PIIPattern(
        name="ip",
    ...
    def detect(self, content: str) -> list[PIIMatch]:
        ...
        for pattern, _ in self._patterns:
            for match in pattern.pattern.finditer(content):
                matches.append(
                    PIIMatch(text=match.group(), start=match.start(), end=match.end(), pii_type=pattern.name)
                )
        return matches
        ...
        for key, value in data.items():
            if isinstance(value, str):
                ...
                result[key] = [
                    ...
                    else self.anonymize_dict(item, recursive)
