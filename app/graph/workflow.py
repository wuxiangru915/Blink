...
async def human_review_node(state: AgentState) -> Command[Literal["extract_visuals", "write_draft", "__end__"]]:
    """
...
def build_workflow_graph() -> StateGraph:
    ...
    # topic_selection -> retrieve (选题完成后检索参考内容)
    workflow.add_edge("topic_selection", "retrieve")

    # retrieve -> write_draft (检索完成后进入写作)
    workflow.add_edge("retrieve", "write_draft")

    # write_draft -> evaluate (写作完成后评估质量)
    workflow.add_edge("write_draft", "evaluate")

    # evaluate -> human_review (评估完成后人工审稿)
